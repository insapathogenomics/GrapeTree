...
"""
This is the script used to run grapetree (May 2021).
For research purposes, this script was modified in order to get MSTreeV2 distances and a list with redundant samples info

By Veronica Mixao
@INSA

python MSTrees.py -p profiles.tab -n 2 -o profileData_TEST
"""
...
def add_args() :
    ...
    args.out = args.tag_output #EDITED
...
class methods(object) :
    ...
    @staticmethod
    def MSTree(output, names, profiles, embeded, matrix_type='asymmetric', heuristic='harmonic', branch_recraft=True, handle_missing='pair_delete', **params) :
        ...
        # output file for partitions ----------
        
        sample_conversion = {} #EDITED
        i = 0 #EDITED
        for name in names: #EDITED
            sample_conversion[i] = name #EDITED
            i += 1 #EDITED
...
def nonredundant(output, names, profiles) :
    users_inter = np.nonzero(profiles)[0]
    ...
    with open(output + "_redundantSamples.txt", "w+") as out_red: #EDITED
        for rep in embeded.keys(): #EDITED
            print(rep + "\t" + ",".join(embeded[rep]), file = out_red) #EDITED
...
def backend(**args) :
    ...
    names, profiles, embeded = nonredundant(params["out"], np.array(names), np.array(profiles)) #EDITED
    ...
    with tempfile.NamedTemporaryFile(delete=True, dir='.') as f :
        ...
        if params['method'] == "MSTree": #EDITED
            tre = eval('methods.' + params['method'])(params["out"], names, profiles, embeded, **params) #EDITED
        else: #EDITED
            tre = eval('methods.' + params['method'])(names, profiles, embeded, **params) #EDITED
